...
function GameLibrary() {
  ...
  const filtered = useMemo(() => {
    ...
    if (showFavoritesOnly) list = list.filter((g) => favorites.includes(g.id));
    ...
    return list;
  }, [search, genre, platform, sort, showFavoritesOnly, favorites]);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-white/50">
          {filtered.length} of {games.length} games owned across your connected platforms
        </p>
      </div>
      ...
      {filtered.length === 0 ? (
        <p className="text-center text-sm text-white/45 py-12">No games match your filters.</p>
      ) : (
